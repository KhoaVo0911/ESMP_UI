...
const BoothPayment = ({
      ...
      try {
        ...
        if (hostData) {
          ...
          if (apibanking && bankingaccount) {
            ...
            const apiBankingResponse = await axios.post("/host/apibanking", {
              apibanking,
            });
  ...
  const checkPaid = async (price, content, startTime) => {
    ...
    try {
      ...
      console.error("An error occurred while checking the payment.", error);
      // toast({
      //   title: "Error",
  ...
  return (
    <VStack align="center" spacing={6} w="full" pb={10}>
      <Text>Please scan the QR code below to complete your payment.</Text>
