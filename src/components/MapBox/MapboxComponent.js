...
const MapboxComponent = ({ eventId, eventData = {}, onSaveCoordinates }) => {
  ...
  const handleSubmitToBackend = async () => {
    ...
    if (coordinates === eventData.coordinates) {
      alert("No changes detected in coordinates!");
      return;
    }
    ...
    try {
      const payload = {
        ...
        onWed: eventData.onWeb, // Ensure onWeb is mapped to onWed
      };
    ...
    >
      <VStack spacing={4} align="stretch">
        ...
        <Button
          onClick={handleSubmitToBackend}
          colorScheme="green"
          isDisabled={!coordinates || isUpdating}
          width="full"
        >
          {isUpdating ? "Updating..." : "Update Event Location"}
        </Button>
      </VStack>
    </Box>
  );
